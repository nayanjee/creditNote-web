...
export class ClaimApprovalComponent implements OnInit {
  ...
  faStar = faStar;
  faPlus = faPlus;
  heading = 'Claims for Approval';
  subheading = 'Approve or reject stockiest claim.';
  ...
  ngOnInit() {
    ...
    this.delay(1000).then(any => {
      ...
      const invoice = this.activatedRoute.snapshot.paramMap.get('invoice');
      this.getData(stockiest, month, year, type, invoice);
    });
  ...
  getRemaining(record, recordIndex, rid) {
    return new Promise(async resolve => {
      ...
      const remainingQuantity: any = await this.remainingQuantity(reqData);
      console.log('remainingQuantity--', remainingQuantity);

      if (remainingQuantity) {
        let allotedQty = 0;
        remainingQuantity.forEach(async(remaining) => {
          // console.log('remaining--', remaining);
          
          const requiredQty = record.saleQuantity - allotedQty;
          // console.log('requiredQty---', requiredQty);

          if(allotedQty >= record.saleQuantity) {
            // console.log('Break foreach');
            return; // Break foreach loop
          } else {
            let allotQty = 0;

            const remainingStock = (remaining.remainingData.length) ? remaining.remainingData.quantity : remaining.saleUnit;
            // console.log('remainingStock---', remainingStock);

            if (remainingStock >= record.saleQuantity) {
              // console.log('Greater and Equal >=');
              allotQty = requiredQty;
            } else {
              // console.log('Less quantity', record.saleQuantity);
              allotQty = remainingStock;
            }
            allotedQty = allotedQty + allotQty;
            // console.log('allotQty---', allotedQty);

            // To remove current row (if it new row)
            if (rid) {
            // if (rid && allotedQty >= record.saleQuantity) {
              this.removeClaim(recordIndex);
            }

            const margin = record.margin ? record.margin : 10;

            this.fileNames[recordIndex] = record.files;
            const claimData = {
              ...
              difference: (remaining.pts - record.billingRate).toFixed(2),
              totalDifference: ((remaining.pts - record.billingRate) + (record.billingRate * margin / 100)).toFixed(2),
              amount: (((remaining.pts - record.billingRate) + (record.billingRate * 10 / 100)) * allotQty).toFixed(2),
              totalSaleQuantity: allotQty,
              ptd: remaining.ptd,
              totalPtdAmt: (remaining.ptd * allotQty).toFixed(2),
              image: ''
            };
            const fbClaimData = this.fb.group(claimData);
            this.claims().push(fbClaimData);
  ...
  async getData(stockiest, month, year, type, invoice) {
    ...
    const claimForApproval = await this.getClaimForApproval(requestData);
    // console.log('claimForApproval--', claimForApproval);

    this.records = claimForApproval;
    console.log('records--', this.records);
    if (this.records.length) {
      this.records.forEach(async (record, recordIndex) => {
        const result = this.getRemaining(record, recordIndex, '');
        console.log('result--', result);
      });
    }
    
    
    /* this.apiService.post('/api/getClaimForApproval', requestData).subscribe((response: any) => {
      if (response.status === 200) {
        if (response.data.length) {
          ...
          this.records.forEach((element, index) => {
            ...
            this.fileNames[index] = element.files;
            
            const reqData = {
              customerId: element.customerId,
              batch: element.batch
            };
            this.apiService.post('/api/sales/remainingQuantity', reqData).subscribe((resp: any) => {
              console.log('resp.data---', resp.data);
              let allotQty = 0;
              resp.data.forEach(async(ele) => {
                console.log('---', index, ele.material, ele.customer);

                let hoSales: any = [];
                let distInvoice = '';
                let distInvQty = '';

                if (ele.customer.length) {
                  let custIds = [];
                  ele.customer.forEach(el => {
                    console.log('customerId---', el.customerId);
                    custIds.push(el.customerId);
                  });
                  console.log('custids--', custIds);

                  hoSales = await this.getHoSales(ele.batch, custIds);
                  console.log('hoSales--', hoSales);
                }

                if (hoSales.status === 200 && hoSales.data.length) {
                  distInvoice = hoSales.data[0].billDocNumber;
                  distInvQty = hoSales.data[0].salesQty;
                }
                console.log('distInvoice--', distInvoice);

                if (ele.remainingData.length) {
                  console.log('exists');
                } else {
                  console.log('not exists', element.saleQuantity, ele.salesQty);
                  if (element.saleQuantity <= ele.salesQty) {
                    console.log('yahooooooooooo', ele.billDocNumber, element.ptd, element.saleQuantity);
                    const claimData = this.fb.group({
                      ...
                      particulars: '',
                      category: '',
                      distInvoice: distInvoice,
                      distInvQty: distInvQty,
                      stkInvoice: ele.billDocNumber,
                      stkInvoiceQty: ele.salesQty,
                      stkRemnQty: ele.salesQty - element.saleQuantity,
                      mrp: element.mrp,
                      pts: element.pts,
                      ptd: ele.ptd,
                      totalPtdAmt: ele.ptd * element.saleQuantity,
                      billingRate: element.billingRate,
                      margin: element.margin,
                      freeQuantity: element.freeQuantity,
                      saleQuantity: element.saleQuantity,
                      totalSaleQuantity: element.saleQuantity,
                    ...
                    console.log('Need to develop code');
                  }

                }
                // const remainingStock = (ele.remainingData.length) ? ele.remainingData[0].quantity : ele.salesQty;
                // console.log('----------------------------------------------------', remainingStock);

                // if (remainingStock >= element.saleQuantity) {
                //   console.log('Greater and Equal >=');
                // } else {
                //   console.log('Less quantity', element.saleQuantity);
                //   const requiredQty = element.saleQuantity - allotQty;
                //   console.log('requiredQty---', requiredQty);
                //   if (remainingStock >=  requiredQty) {
                //     allotQty = allotQty + (remainingStock - requiredQty);
                ...
                //       margin: element.margin,
                //       freeQuantity: element.freeQuantity,
                //       saleQuantity: remainingStock - requiredQty,
                //       difference: element.difference,
                //       totalDifference: element.totalDifference,
                //       amount: element.amount,
                //       image: ''
                ...
                //     this.claims().push(claimData);
                //   } else {
                //     allotQty = allotQty + remainingStock;
                ...
                //       material: ele.material,
                //       particulars: '',
                //       distInvoice: ele.billDocNumber,
                //       mrp: element.mrp,
                //       pts: element.pts,
                //       billingRate: element.billingRate,
                //       margin: element.margin,
                //       freeQuantity: element.freeQuantity,
                //       saleQuantity: remainingStock,
                //       difference: element.difference,
                //       totalDifference: element.totalDifference,
                //       amount: element.amount,
                //       image: ''

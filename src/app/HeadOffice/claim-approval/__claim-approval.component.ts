...
export class ClaimApprovalComponent implements OnInit {
  ...
  async getData(stockiest, month, year, type, invoice) {
    ...
    if (this.records.length) {
      this.records.forEach(async (element, index) => {
        ...
        if (remainingQuantity) {
          ...
          remainingQuantity.forEach(async(ele) => {
            console.log('ele--', ele);
            
            // Get HO-Distributor sales invoice
            let distInvoice = 0;
            let distInvQty = 0;
            let hoSales: any = [];

            if (ele.distCustomerIds.length) {
              let distCustomerIds = [];
              ele.distCustomerIds.forEach(el => {
                distCustomerIds.push(el.customerId);
              });

              hoSales = await this.getHoSales(ele.batch, distCustomerIds);
              console.log('hoSales--', hoSales);
            }

            if (hoSales.status === 200 && hoSales.data.length) {
              distInvoice = hoSales.data.billDocNumber;
              distInvQty = hoSales.data.salesQty;
            }
            // EOF Get HO-Distributor sales invoice

            const remainingStock = (ele.remainingData.length) ? ele.remainingData.quantity : ele.saleUnit;
            console.log('--------------------------', remainingStock, element.saleQuantity);

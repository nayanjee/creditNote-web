...
export class ClaimApprovalComponent implements OnInit {
  ...
  checkQuantity(e, row) {
    ...
    const remainingStock = parseInt($('#hdn_stkRemnQty_' + row).val());

    if (remainingStock >= saleQuantity) {
      const remaining = remainingStock - saleQuantity;
      $('#stkRemnQty_' + row).val(remaining);
      $('#totalSaleQuantity_' + row).val(saleQuantity);
    } else {
      $('#stkRemnQty_' + row).val(remainingStock);
  ...
  async getData(stockiest, month, year, type, invoice) {
    ...
    if (this.records.length) {
      this.records.forEach(async (record, recordIndex) => {
        this.getRemaining(record, recordIndex);
      });
    }
  }

  async getRemaining(demand, demandIndex) {
    const reqData = {
    ...
    if (salesAndRemainingQuantity) {
      ...
      salesAndRemainingQuantity.forEach(async (remaining) => {
        ...
        if (allotedQty >= demand.saleQuantity) {
          ...
          //console.log('remainingStock---', remainingStock, remaining.saleUnit);
          //console.log('nayan---', typeof remainingStock, typeof demand.saleQuantity)

          if (remainingStock >= demand.saleQuantity) {
            //console.log('Greater and Equal >=');
            allotQty = requiredQty;
          } else {
            //console.log('Less quantity', demand.saleQuantity);
          ...
          //console.log('allotedQty---', allotedQty);
          
          const margin = demand.margin ? demand.margin : 10;

          this.fileNames[demandIndex] = demand.files;
          const claimData = {
            ...
            amount: (((remaining.pts - demand.billingRate) + (demand.billingRate * 10 / 100)) * allotQty).toFixed(2),
